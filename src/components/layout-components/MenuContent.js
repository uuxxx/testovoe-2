...
function SideNavContent(props) {
  const {
    sideNavTheme, routeInfo, hideGroupTitle, localization, onMobileNavToggle,
  } = props;
  ...
  return (
    ...
    >
      {navigationConfig.map((menu) => (menu.submenu.length > 0 ? (
        ...
        >
          {menu.submenu.map((subMenuFirst) => (subMenuFirst.submenu.length > 0 ? (
            <SubMenu
              icon={
                    subMenuFirst.icon ? (
